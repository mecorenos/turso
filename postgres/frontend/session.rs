...
use crate::aliases;
use crate::catalog::{self, PostgresSchemaDialect};
...
impl PgConnection {
    pub fn new(conn: Arc<Connection>) -> Self {
        conn.install_schema_dialect(Arc::new(PostgresSchemaDialect));
        catalog::install(&conn).expect("postgres catalog installation should succeed");
...
fn prepare_statement(conn: &Arc<Connection>, sql: &str) -> Result<Statement> {
    ...
    for prereq in translated.prereqs {
        let mut stmt = conn.prepare_stmt(prereq)?;
        stmt.run_ignore_rows()?;
    }
