...
pub fn translate_create_table(
    ...
    // the schema dialect format the SQL to store (the SQLite dialect stores
    // the canonical SQLite form, a frontend dialect preserves its own).
    let sql = if let Some(ref info) = ctas_info {
        info.schema_sql.clone()
    } else {
        connection
            .schema_dialect()
            .to_sql(input, &tbl_name, &body)?
    };
