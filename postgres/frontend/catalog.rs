...
use turso_core::{
    ...
    Connection, InternalVirtualTable, InternalVirtualTableCursor, LimboError, Result,
    SchemaDialect, Value, VirtualTable,
};
...
impl SchemaDialect for PostgresSchemaDialect {
    fn parse_sql(&self, sql: &str, root_page: i64) -> Result<BTreeTable> {
    ...
    fn resolve_function(&self, name: &str, arg_count: usize) -> bool {
        crate::functions::resolve_scalar(name, arg_count)
    }

    fn scalar_function(&self, conn: &Connection, name: &str, args: &[Value]) -> Result<Value> {
...
pub fn install(conn: &Connection) -> Result<()> {
    for vtab in pg_catalog_virtual_tables() {
        match conn.register_virtual_table(vtab) {
            Ok(_) => {}
